...
static GpuMesh buildSurfaceMesh(const std::vector<Vec3>& points, const std::vector<Face>& faces) {
...
// Aristas UNICAS de todos los tetraedros (para ver la estructura interna).
static GpuMesh buildTetraEdgesMesh(const std::vector<Vec3>& points, const std::vector<Tetrahedron>& tets) {
...
static void deleteMesh(GpuMesh& m) {
