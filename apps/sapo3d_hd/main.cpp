...
// Convierte HSV (h en grados 0-360, s,v en 0-1) a RGB. Solo se usa como
// color INICIAL por organo -- despues el usuario lo puede editar en ImGui.
...
// Carga desde cache si existe; si no, triangula desde el .xyz y GUARDA
// el cache para la proxima corrida del programa.
static void ensureTriangulated(OrganState& o, BowyerWatsonStrategy& strategy,
    ...
    if (TetraCache::load(cachePath, o.points, o.tets)) {
        o.triangulated = true;
        o.fromCache = true;
        return;
    }
...
int main() {
    Application app(1400, 900, "cg-delaunay-sapo3d :: Sapo HD (cache)");
    if (!app.ok()) return -1;
    ...
    const std::string cacheDir = "data/cache";
    ...
    app.run([&](float dt) {
        ...
        ImGui::Begin("Sapo 3D HD - Organos (con cache)");
        ...
        if (!manifestError.empty()) {
            ...
            ImGui::Text("%d organos disponibles  (cache: %s)", (int)organs.size(), cacheDir.c_str());
            ...
            for (auto& o : organs) {
                ...
                // Editable: clic abre el picker de color, no solo un swatch.
